...
CORRECTED (PA001-M1): pyiceberg 0.12 CAN expire snapshots, through
`table.maintenance.expire_snapshots()`. This paragraph used to say it
had "no snapshot expiry at all"; that was checked through the wrong
entry point and was wrong, and five other places in the repository
repeated it.

Gold expires its own snapshots now, under a seven-day age margin
(core/mirror/gold.py, tests/unit/test_snapshot_expiry.py). Bronze does
not yet: its retention is still declared properties only.

WHAT IS STILL TRUE is the distinction this paragraph was making, for a
narrower reason: expiry reclaims METADATA, not disk. The Parquet files
of an expired snapshot stay until an orphan sweep removes them, and
pyiceberg has none (apache/iceberg-python #3361). Measured: 15
snapshots to 3, files unchanged at 10, directory 7.7 KB larger.
