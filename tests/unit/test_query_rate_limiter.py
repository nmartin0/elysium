...
@pytest.fixture
def store(tmp_path: Path) -> QueryRateLimiter:
    return QueryRateLimiter(tmp_path / "credentials.db")


def test_user_with_no_record_at_all_is_not_rate_limited(store):
    assert store.is_rate_limited("alice") is False


def test_not_rate_limited_after_fewer_than_max_queries(store):
    for _ in range(MAX_QUERIES_PER_WINDOW - 1):
        store.record_query("alice")
    assert store.is_rate_limited("alice") is False


def test_rate_limited_after_exactly_max_queries(store):
    for _ in range(MAX_QUERIES_PER_WINDOW):
        store.record_query("alice")
    assert store.is_rate_limited("alice") is True


def test_stays_rate_limited_after_further_queries_recorded_anyway(store):
    for _ in range(MAX_QUERIES_PER_WINDOW + 3):
        store.record_query("alice")
    assert store.is_rate_limited("alice") is True


def test_queries_for_one_user_do_not_affect_another(store):
    for _ in range(MAX_QUERIES_PER_WINDOW):
        store.record_query("alice")

    assert store.is_rate_limited("alice") is True
    assert store.is_rate_limited("bob") is False


def test_the_limit_expires_once_the_window_has_passed(store, tmp_path: Path):
    for _ in range(MAX_QUERIES_PER_WINDOW):
        store.record_query("alice")
    assert store.is_rate_limited("alice") is True
...
def test_a_query_after_the_window_expired_starts_a_fresh_count(store, tmp_path: Path):
    for _ in range(MAX_QUERIES_PER_WINDOW):
        store.record_query("alice")
