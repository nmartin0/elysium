...
class MirrorReadAdapter(ExternalReadAdapter):
    ...
    def __init__(self, catalog: SqlCatalog, silo_name: str,
        ...
        # words a source writes for true and false (F-01) are shared
        # with the gold connector, which reads a different namespace
        # with the same machinery. Two copies would keep whichever
        # bug the copy was made before. See iceberg_reader.py.
        self._reader = IcebergNamespaceReader(catalog, silo_name, snapshot_ids)
    ...
    def find_ids_matching_text(self, object_type: str, columns: list[str], query_text: str,
        ...
        arrow = self._reader._scan(table_name, selected_fields=(id_column, *columns))
        if arrow is None:
            return []
    ...
    def read_fields_for_ids(self, table_name: str, id_column: str, object_ids: list,
        ...
        if not object_ids:
            return []
        arrow = self._reader._scan(table_name, selected_fields=tuple(dict.fromkeys([id_column, *columns])))
        if arrow is None:
            return []
        wanted = {str(object_id) for object_id in object_ids}
